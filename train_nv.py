...
def clone_as_averaged_model(device, model, ema):
    ...
    averaged_model = build_model().cuda(device)
...
def eval_model(global_step, writer, device, model, y, c, g, input_lengths, eval_dir, ema=None):
    ...
    if is_mulaw_quantize(hparams.input_type):
        ...
        initial_input = torch.from_numpy(initial_input).view(1, 1, hparams.quantize_channels)
    else:
        initial_input = torch.zeros(1, 1, 1).fill_(initial_value)
    initial_input = initial_input.cuda(device)
...
def __train_step(device, phase, epoch, global_step, global_test_step,
    ...
    input_lengths = input_lengths.cuda(device)
    c = c.cuda(device) if c is not None else None
    g = g.cuda(device) if g is not None else None
...
if __name__ == "__main__":
    ...
    args = {
        ...
        "--log-event-path": 'log_nv_8bit',
        "--data-root": None,
        "--reset-optimizer": None
    }
    print("Command line args:\n", args)
    # checkpoint_dir = args["--checkpoint-dir"]
    checkpoint_dir = 'checkpoints-8bits_nv'
    ...
    if data_root is None:
        data_root = join(dirname(__file__), "data", "ljspeech_8bits")
    ...
    # model.compute_condition_input(c,g=None)
    ####
